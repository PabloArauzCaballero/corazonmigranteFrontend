      ...
      <div className="grid gap-6">
        ...
        {companies.data ? (
          <div className="grid gap-4">
            <DataTable<AdsCompany> data={companiesPaged.rows} getRowKey={(row) => row.id} columns={[{ key: "logo", header: "Logo", render: (row) => { const url = (row.metadata as Record<string, unknown> | undefined)?.logoUrl as string | undefined; return url ? <img src={url} alt={row.commercialName} className="h-10 w-10 rounded-lg border object-cover" onError={(e) => { e.currentTarget.style.display = "none"; }} /> : <span className="grid h-10 w-10 place-items-center rounded-lg border bg-muted text-muted-foreground"><Megaphone className="h-4 w-4" /></span>; } }, { key: "name", header: "Empresa", render: (row) => <div><b>{row.commercialName}</b><p className="text-xs text-muted-foreground">{row.businessName}</p></div> }, { key: "contact", header: "Contacto", render: (row) => <span>{row.contactEmail ?? "—"}<br />{row.contactPhone ?? "—"}</span> }, { key: "status", header: "Estado", render: (row) => <StatusBadge status={row.status} /> }]} />
